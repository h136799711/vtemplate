...
export default {
  ...
  methods: {
    onBack () {
      this.$router.replace({ path: '/admin/region/city_area/' + this.parent_code })
    },
      ...
      this.$confirm(this.$i18n.t('Action Confirm'), this.$t('Alert'), {
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            regionApi.deleteTown({ id: id }, (res) => {
    ...
    submitEditForm () {
      this.loading = true
      let that = this
      regionApi.updateTown(this.editForm, (resp) => {
        that.loading = false
    ...
    submitAddForm () {
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          regionApi.createTown(this.addForm, (resp) => {
            this.loading = false
    ...
    async refresh () {
      ...
      if (this.parent_code === '') {
        let info = await regionApi.infoCityArea(this.code)
        this.parent_code = info.parent_code
      }
      regionApi.queryTown(this.queryForm, (resp) => {
