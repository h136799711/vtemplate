...
export default {
  ...
  methods: {
    onBack () {
      this.$router.replace({ path: '/admin/region/province/' + this.parent_code })
    },
      ...
      this.$confirm(this.$i18n.t('Action Confirm'), this.$t('Alert'), {
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            regionApi.deleteCity({ id: id }, (res) => {
    ...
    onSubregion (row) {
      this.$router.push({ path: '/admin/region/city_area/' + row.code })
    },
    submitEditForm () {
      this.loading = true
      let that = this
      regionApi.updateCity(this.editForm, (resp) => {
        that.loading = false
    ...
    submitAddForm () {
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          regionApi.createCity(this.addForm, (resp) => {
            this.loading = false
    ...
    async refresh () {
      ...
      if (this.parent_code === '') {
        let info = await regionApi.infoProvince(this.code)
        this.parent_code = info.country_id
      }
      regionApi.queryCity(this.queryForm, (resp) => {
